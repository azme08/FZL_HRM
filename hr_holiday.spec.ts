...
test.describe('Holidays layout test cases', () => {
    ...
    test('should verify Holidays Dashboard Functionality', async ({ page }) => {
        // Click "HR"
        ...
        // Click "Holidays"
        const layout = page.getByText('Holidays', { exact: true }).first();
        await expect(layout).toBeVisible({ timeout: TEST_TIME['5min'] });
        await layout.click();
        // Check the page header
        const pageHeader = page.getByRole('heading', { name: 'HR/Holidays' });
    ...
    test('should verify update Holidays Functionality', async ({ page }) => {
        // Click "HR"
        const hr = page.getByText('HR', { exact: true }).first();
        await expect(hr).toBeVisible({ timeout: TEST_TIME['5min'] });
        await hr.click();
        ...
        await settings.click();
        // Click "Holidays"
        const designation = page.getByText('Holidays', { exact: true }).first();
        ...
        const updateTitle = page.getByText('Update Holiday', { exact: true });
    ...
    test('should verify create Holidays Functionality', async ({ page }) => {
        // Click "HR"
        const hr = page.getByText('HR', { exact: true }).first();
        await expect(hr).toBeVisible({ timeout: TEST_TIME['5min'] });
        await hr.click();
        ...
        // Name Input
        await fillInputByLabel(page, /name/i, 'Meeting Day');
        // Date input
        await page.getByRole('gridcell', { name: '23' }).click();
        // Remark Input
        await fillInputByLabel(page, /remark/i, 'Test Remark');
