...
test.describe('Designation layout test cases', () => {
    ...
    test('should verify Designation Dashboard Functionality', async ({ page }) => {
        // Click "HR"
        ...
        // Click "Designation"
        const designation = page.getByText('Designation', { exact: true }).first();
        await expect(designation).toBeVisible({ timeout: TEST_TIME['5min'] });
        await designation.click();
        // Check the page header
        const pageHeader = page.getByRole('heading', { name: 'HR/Designation' });
        ...
        // Designation Input
        await fillFilterInput(filterPanel, 'Manager', 1);
        // remark Input
    ...
    test('should verify update Designation Functionality', async ({ page }) => {
        // Click "HR"
        const hr = page.getByText('HR', { exact: true }).first();
        await expect(hr).toBeVisible({ timeout: TEST_TIME['5min'] });
        await hr.click();
        ...
        await sortTableByColumn(
            page,
            table,
            'Designation', // Column name to sort by
        ...
        const updateTitle = page.getByText('Update Designation', { exact: true });
        await expect(updateTitle).toBeVisible({ timeout: TEST_TIME['10min'] });
        // Name Input
        await fillInputByLabel(page, /name/i, 'Test Designation');
        // Hierarchy Input
    ...
    test('should verify create Designation Functionality', async ({ page }) => {
        // Click "HR"
        const hr = page.getByText('HR', { exact: true }).first();
        await expect(hr).toBeVisible({ timeout: TEST_TIME['5min'] });
        await hr.click();
        ...
        // Name Input
        await fillInputByLabel(page, /name/i, 'Test Title');
        // Hierarchy Input
        await fillInputByLabel(page, /hierarchy/i, '1');
        // Remark Input
        await fillInputByLabel(page, /remark/i, 'Test Remark');
