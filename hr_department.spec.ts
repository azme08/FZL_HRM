...
test.describe('Department layout test cases', () => {
    ...
    test('should verify Department Dashboard Functionality', async ({ page }) => {
        // Click "HR"
        ...
        // Click "Department"
        const designation = page.getByText('Department', { exact: true }).first();
        ...
        // Department Input
        await fillFilterInput(filterPanel, 'IT', 1);
        // remark Input
    ...
    test('should verify update Department Functionality', async ({ page }) => {
        // Click "HR"
        const hr = page.getByText('HR', { exact: true }).first();
        await expect(hr).toBeVisible({ timeout: TEST_TIME['5min'] });
        await hr.click();
        ...
        const updateTitle = page.getByText('Update Department', { exact: true });
        await expect(updateTitle).toBeVisible({ timeout: TEST_TIME['10min'] });
        // Name Input
        await fillInputByLabel(page, /name/i, 'Test Department');
        // Hierarchy Input
    ...
    test('should verify create Department Functionality', async ({ page }) => {
        // Click "HR"
        const hr = page.getByText('HR', { exact: true }).first();
        await expect(hr).toBeVisible({ timeout: TEST_TIME['5min'] });
        await hr.click();
        ...
        // Name Input
        await fillInputByLabel(page, /name/i, 'Test Dept');
        // Hierarchy Input
        await fillInputByLabel(page, /hierarchy/i, '1');
        // Remark Input
        await fillInputByLabel(page, /remark/i, 'Test Remark');
